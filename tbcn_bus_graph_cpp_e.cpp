...
int main(void){
    ...
    //////////
    std::cout << "digraph G{graph[forcelabels=true;layout=neato;size=\"40,40\";fontsize=100;outputorder=edgesfirst;penwidth=10;sep=0];\
    ...
    for(int i=0;i<narabijyunn_line-1;++i){
        if(n_narabijyunn[i] < n_narabijyunn[i+1]){
            std::cout << "\"" << n_basutei[i] << "\"->\"" << n_basutei[i+1] << "\";";
        };
    };
    //////////
    std::cout << "}";
